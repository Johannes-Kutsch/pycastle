...
The status-row counter denominator (`Y` in `"started Agents for X/Y issues"`) is the full planner output, not `max_parallel`. `X` increments on each agent-semaphore acquire by an Implementer.
...
- `"Running: started Agents for X/Y issues"`: `Y = len(plan_result.issues)`, `X` increments on each agent-semaphore acquire by an Implementer (Reviewer acquires do not re-fire). Counter terminates at `len(issues)/len(issues)`.
