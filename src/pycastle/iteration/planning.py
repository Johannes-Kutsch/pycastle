...
def _build_planning_readiness(
    ...
    readiness_result = evaluate_planning_readiness(open_issues, cfg)
    slice_well_formed = [
        issue
        for issue in open_issues
        if issue not in readiness_result.malformed_slice_mode_issues
    ]
    label_sync_actions = tuple(
        list(readiness_result.label_sync_actions)
        + _needs_slice_type_actions(
            slice_well_formed,
            list(readiness_result.malformed_slice_mode_issues),
            cfg,
        )
    )
