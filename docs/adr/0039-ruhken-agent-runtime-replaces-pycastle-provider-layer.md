...
**Tool policy.** Ar's `ToolPolicy` enum (`NONE`, `NO_FILE_MUTATION`, `UNRESTRICTED`) replaces `flag_profiles.py`. The Planner uses `NO_FILE_MUTATION`; all other roles, including the Divergence-Resolver, use `UNRESTRICTED`. Per-service CLI flag matrices are deleted; ar manages provider-specific flag construction internally. *(Amended by ADR 0062: Divergence-Resolver moved from `NO_FILE_MUTATION` to `UNRESTRICTED`.)*
