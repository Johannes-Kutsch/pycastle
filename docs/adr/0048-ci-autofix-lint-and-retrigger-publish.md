...
- **Tag invariant / simultaneous push:** the intended release workflow is `git push origin main vX.Y.Z` in one command — main and the tag land together. The tag branch in `ci-autofix.sh` therefore pushes to `main` **first**, then force-moves the tag. If the main push is rejected (race lost to the concurrent main-branch autofix run), the script fetches the updated `origin/main`, rebases the local autofix commit onto it, re-tags HEAD, and retries the main push. This makes the race self-healing: the tag is force-moved to the rebased (or already-landed) autofix commit and the re-triggered workflow publishes to PyPI at the correct version. If the rebase itself has a conflict the script exits non-zero and leaves the remote tag unchanged; that case requires manual recovery: `git fetch --tags && git tag -f vX.Y.Z origin/main && git push origin vX.Y.Z --force`.
...
- Near-simultaneous pushes (main + tag together) can race: both workflows start seconds apart and both try to push a ruff-fix commit. The push-order rule (main before tag) resolves this: the losing (tag) runner fetches the winner's commit, rebases its autofix onto it, and force-moves the tag — a fully self-healing path. The only failure mode is a rebase conflict, which is expected to be extremely rare since both runners apply the same mechanical ruff fix to the same files.
