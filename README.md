...
stateDiagram-v2
    ...
    needs_triage --> needs_info : more detail required
    needs_info --> needs_triage : info provided
    needs_triage --> ready_for_agent : triaged and specified
    needs_triage --> wontfix : out of scope
    needs_info --> wontfix : abandoned
    ready_for_agent --> ready_for_human : preflight fails (needs human)
    ready_for_agent --> [*] : implemented and merged
    ready_for_human --> ready_for_agent : human resolves
    wontfix --> [*] : closed

    state "needs-triage" as needs_triage
    state "needs-info" as needs_info
    state "ready-for-agent" as ready_for_agent
    state "ready-for-human" as ready_for_human
