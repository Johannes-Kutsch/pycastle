...
`ContainerRunner.work()` previously accepted nine parameters spanning two concerns: prompt rendering (`template`, `scope_args`, `renderer`, `send_role_prompt_on_resume`) and execution context (`role`, `run_kind`, `session_uuid`, `is_failsoft_recovery`). Inside `work()`, a three-way branch over `(run_kind, send_role_prompt_on_resume, is_failsoft_recovery)` decided which template to render — meaning the runner had to understand improve-mode session namespace semantics (ADR 0008) to decide what to write to `/tmp/.pycastle_prompt`.
...
- **Two cohesive structs (`PromptSpec` + `WorkContext`).** Rejected: preserves runner→ADR-0008 coupling and the three-way branch in the hardest place to test.
...
- ADR 0007's `PromptRenderer` shape unaffected; sole caller moves to `AgentRunner._build_prompt()`. ADR 0008's session-namespace semantics no longer need to be understood by `ContainerRunner`.

> **Note (ADR 0039/0040).** The `ContainerRunner.work()` seam described here was later removed when ar took over provider-subprocess execution; the durable decision — prompt-shape resolution lives upstream of the container substrate, not inside it — still holds via the prompt dispatch module.
