...
def test_create_worktree_does_not_recreate_valid_ancestor_branch(git_repo, tmp_path):
    ...
    (git_repo / "pyproject.toml").write_text("[project]\nname = 'test'\n")
    subprocess.run(["git", "-C", str(git_repo), "add", "pyproject.toml"], check=True, capture_output=True)
    subprocess.run(["git", "-C", str(git_repo), "commit", "-m", "add project files"], check=True, capture_output=True)
    ...
    subprocess.run(["git", "-C", str(git_repo), "commit", "-m", "extra commit"], check=True, capture_output=True)
    ...
    assert branch_tip_before == branch_tip_after, "branch must not be recreated when project files are present"


def test_create_worktree_raises_when_non_ancestor_branch_has_no_project_files(git_repo, tmp_path):
    ...
    subprocess.run(
        ["git", "-C", str(git_repo), "worktree", "add", str(temp_wt), "issue/2-real-work"],
        check=True, capture_output=True,
    )
    (temp_wt / "implementer_work.txt").write_text("real work")
    subprocess.run(["git", "-C", str(temp_wt), "add", "implementer_work.txt"], check=True, capture_output=True)
    subprocess.run(["git", "-C", str(temp_wt), "commit", "-m", "real implementer work"], check=True, capture_output=True)
    subprocess.run(
...
def test_patch_gitdir_rewrites_windows_path(tmp_path):
    """On Windows the .git file must use the container-internal gitdir path."""
    worktree = tmp_path / "my-branch"
    worktree.mkdir()
    (worktree / ".git").write_text(
        "gitdir: C:/Users/johan/repo/.git/worktrees/my-branch\n"
    )

    with patch("sys.platform", "win32"):
        patch_gitdir_for_container(worktree)

    result = (worktree / ".git").read_text().strip()
    assert result == "gitdir: /home/agent/repo/.git/worktrees/my-branch"
...
def test_patch_gitdir_retries_on_transient_permission_error(tmp_path):
    """A single PermissionError on write must not propagate — the function must retry."""
    worktree = tmp_path / "my-branch"
    worktree.mkdir()
    ...
    def _flaky_write(self, data, **kwargs):
        attempt[0] += 1
        if attempt[0] == 1:
            raise PermissionError("file locked by git.exe")
        return _real_write(self, data, **kwargs)

    with patch("sys.platform", "win32"), patch.object(Path, "write_text", _flaky_write):
        patch_gitdir_for_container(worktree)
