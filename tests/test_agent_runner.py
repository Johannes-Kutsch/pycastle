...
class _WorkProtocolRetryRuntimeClient:
    def __init__(self) -> None:
        self.prompts: list[str] = []
    ...
    async def run_new_session(self, request):
        ...
        if len(self.prompts) == 1:
            return RuntimeOutcome(
                kind=Completed(),
                result=RunResult(
                    output="implemented the change but forgot the protocol tag",
                    usage=None,
                    continuation=None,
                    selected=ResolvedProvider(
                        service="codex",
...
def test_agent_runner_retries_missing_commit_message_for_behavior_prompt_with_template_specific_protocol_correction(
    tmp_path,
    monkeypatch,
):
    mount_path = tmp_path / "repo" / "pycastle" / ".worktrees" / "issue-1930"
    mount_path.mkdir(parents=True)
    ...
    status_display = RecordingStatusDisplay()
    runtime_client = _WorkProtocolRetryRuntimeClient()

    invocation = PromptInvocation(
        template=PromptTemplate.IMPLEMENT_BEHAVIOR,
        scope_args={
            "ISSUE_NUMBER": "1930",
            "ISSUE_TITLE": "Plan work-family protocol correction prompts",
            "ISSUE_BODY": "Body",
            "ISSUE_COMMENTS": "",
            "BRANCH": "pycastle/issue-1930",
            "INTERRUPTED_WORK": "",
        },
    )
    ...
    monkeypatch.setattr(
        runner,
        "_render_runtime_prompt",
        AsyncMock(return_value="initial behavior prompt"),
    )
    monkeypatch.setattr(
        "pycastle.infrastructure.container_runner.ContainerRunner.setup",
        AsyncMock(return_value=None),
    ...
    expected_shape = runner._renderer.render_expected_output_shape(
        invocation.template,
        invocation.scope_args,
    )

    assert result == CommitMessageOutput(message="done")
    assert runtime_client.prompts == [
        "initial behavior prompt",
        "Your last response did not include the required protocol output.\n"
        "Please review the task requirements and try again, making sure to include the required output tag.\n"
        "The parser reported the following error:\n"
        "Agent produced no <commit_message>...</commit_message> tag.\n"
        "Output tail: 'implemented the change but forgot the protocol tag'\n"
