...
def test_run_iteration_routes_failure_report_credential_failure_through_shared_terminal_path(
    ...
    deps = dataclasses.replace(deps, status_display=display)
...
def test_run_iteration_extracts_codex_status_from_raw_error_envelope_when_missing_on_error(
    tmp_path, git_svc, github_svc, logger
):
    raw_line = (
        '{"type": "error", "error": {"type": "invalid_request_error", '
...
def test_run_iteration_keeps_none_status_for_boolean_raw_codex_hard_error_status(
    tmp_path, git_svc, github_svc, logger
):
    raw_line = (
        '{"type": "error", "error": {"type": "invalid_request_error", '
        '"message": "Bad request: invalid model"}, "status": true}'
    )
...
def test_run_iteration_keeps_raw_title_when_error_data_dict_lacks_message(
    tmp_path, git_svc, github_svc, logger
):
    raw_line = (
