...
- Runtime execution is split into two public surfaces: a stage-aware one-shot call path and a resumable resident-agent path. The one-shot surface accepts unresolved `StageOverride` chains, resolves configured fallback inside runtime, and falls through only on usage-limit or exhaustion; resident execution owns session planning and resume metadata.
