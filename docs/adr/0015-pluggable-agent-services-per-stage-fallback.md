...
- **`RoleSession` splits along two axes.** Stage-completion (service-agnostic) stays: `is_done()` / `mark_done()` / `start_fresh()`. Service resume state moves into the service via `state_dir_relpath` + `is_resumable`. `is_stage_done_for(worktree, role)` stays as pure-filesystem check. **Invariant:** `start_fresh()` wipes the entire role dir (including the service subdir); `AgentRunner._run()` must recreate `state_dir_relpath` after `start_fresh()` so the service home exists before the agent process starts.
