...
- [0053 — Single run command with run-slot locking](0053-single-run-command-with-run-slot-locking.md): `cron` folds into `run` behind a hidden deprecated alias; global run lock is a pure queue, project run marker is presence; acquisition order token → scan → marker; `--ignore-global-lock` skips only the queue.
