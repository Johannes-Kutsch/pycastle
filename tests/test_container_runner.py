...
def test_usage_limit_preserves_worktree(tmp_path):
    """When UsageLimitError is raised, remove_worktree_fn must NOT be called."""
    prompt = tmp_path / "p.md"
    prompt.write_text("test")
...
def test_container_cleaned_up_on_usage_limit(tmp_path):
    """Container __exit__ must be called even when UsageLimitError occurs."""
    prompt = tmp_path / "p.md"
    prompt.write_text("test")
