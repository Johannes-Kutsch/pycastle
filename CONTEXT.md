...
| **StatusDisplay** | Injectable abstraction that owns the live terminal status line; exposes `add_agent`, `update_phase`, `update_message`, `remove_agent`, and `print` methods; backed by a `rich` `Live` display in production and a `NullStatusDisplay` no-op in tests; injected via `Deps` as a separate concern from `Logger` | terminal display, status bar |
| **NullStatusDisplay** | Test/no-op implementation of `StatusDisplay`; all methods are no-ops; `print` falls back to `builtins.print` | — |
| **agent status row** | One headerless line in the `StatusDisplay` live panel representing one active agent; format: `elapsed \| AgentName - Issue Headline \| Phase \| idle Last message…`; elapsed is right-justified so all agent names align across rows; agent name is an OSC 8 clickable link to the log file; elapsed counts up from agent start and never resets; idle resets on each Docker stream chunk; last message is truncated with ellipsis to fit terminal width on one line; the live panel is preceded by one blank line to visually separate it from scrollback output; ordered by orchestration phase (plan → implement → review → merge) then by issue number; disappears when the agent finishes | status entry, agent row |
...
- The **agent status row** is created when the Setup phase begins (log file is also created at this point so the OSC 8 link is always valid); the row is removed when the agent finishes or errors; the `rich` `Live` display is started on the first `add_agent` call and stopped after the last `remove_agent` call; `add_agent` receives `issue_title` so the headline is available from row creation; `update_message` stores the last Docker stream chunk and resets the idle timer.
