...
| **agent status row** | One headerless line in the `StatusDisplay` live panel representing one active agent; format: `elapsed \| RoleName \| idle \| body`; elapsed is dim and right-justified so all role names align across rows; role name is bold and colored by agent role (Planner=blue, Implementer=orange, Reviewer=yellow, Merger=green, Pre-Flight=purple, Pre-Flight Reporter=red), with any numeric part (e.g. `5` in `Implementer #5`) styled cyan; idle is dim; body column shows the current lifecycle phase name (e.g. `Setup`, `Pre-flight`, `Prepare`) for all non-Work states, or the `work_body` string during Work (caller-constructed: `"working on {issue_title}"` for Implementers/Reviewers, `"Creating Plan from {n} issues"` for the Planner, `"Merging {n} Branches"` for the Merger, `"Checking"` for the preflight-checks agent, `"reporting {check_name} issue"` for the preflight-issue agent); elapsed counts up from agent start and never resets; idle resets on each Docker stream chunk; the live panel is preceded by one blank line to visually separate it from scrollback output; ordered by orchestration phase (plan → implement → review → merge) then by issue number; disappears when the agent finishes | status entry, agent row |
