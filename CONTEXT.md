...
| **WorkStream** | Class in `stream_session.py` that converts a raw Docker byte stream into an `AgentOutput`; constructed with a byte-chunk iterator, a log path, an idle timeout, and an `on_chunk: Callable[[], None]` callback; its `run(role, on_turn, usage_limit_patterns) → AgentOutput` method drives a feeder thread, writes each byte chunk to the log file (flushed immediately), calls `on_chunk()` per chunk, detects idle timeouts (raising `AgentTimeoutError`), splits bytes into complete UTF-8 lines, and delegates to `process_stream`; the only caller is `ContainerRunner.run_streaming`, which passes `status_display.reset_idle_timer` as the `on_chunk` callback | stream session, work session |
