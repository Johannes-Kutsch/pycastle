...
> **Amendment (host-owned filing — what phase 03 inlines).** With host-owned filing (ADR 0058), phase 03 no longer receives the PRD via `{{ISSUE_NUMBER}}` / `{{ISSUE_TITLE}}` / `{{ISSUE_BODY}}` / `{{ISSUE_COMMENTS}}` scope placeholders. Instead, phase 02 writes the PRD body to `_drafts/spec.md` at the close of its run; phase 03 reads `_drafts/spec.md` directly from the worktree to get the PRD content it will slice. The `IMPROVE_ISSUES` prompt scope is consequently narrowed to `{IMPROVE_SHORT_SID}` only. The `get_issue` + `get_issue_comments` re-fetch and unconditional re-inlining described above are retired; the agent reads from the draft file rather than from GitHub. The resume-vs-fresh behaviour of phase 03 and the graceful-degrade model are otherwise unchanged.
