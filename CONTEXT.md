...
| **worktree** | An isolated git working tree created on the host and bind-mounted into an agent container; either a named-branch worktree for an Implementer/Reviewer/Merger, or a detached pre-planning worktree for the plan phase | workspace, branch dir |
| **pre-planning worktree** | A temporary detached checkout of the safe SHA created by `plan_phase` at the start of each iteration; shared by the Pre-flight phase, the preflight-issue agent (if spawned), and the Planner; always removed in a `try/finally` by `plan_phase` regardless of state; never associated with a branch | planner worktree, temp worktree |
