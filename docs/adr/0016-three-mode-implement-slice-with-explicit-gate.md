...
- **Three implement prompts:** `implement/behavior.md` (two-phase TDD with gate), `implement/refactor.md` (short — change named symbols, no new tests, feedback commands at end), `implement/docs.md` (shortest — apply markdown edits, no code, no tests). `PromptTemplate` gains `IMPLEMENT_BEHAVIOR`, `IMPLEMENT_REFACTOR`, `IMPLEMENT_DOCS`; singular `IMPLEMENT` removed.
