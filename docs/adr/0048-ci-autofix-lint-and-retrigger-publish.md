...
- **Tag invariant:** tags are always cut from `main`'s current HEAD. Guarantees the fix push to `main` is always a clean fast-forward — never a cherry-pick, never a conflict path in CI.
...
- Near-simultaneous `main` pushes could race on the fix push (non-fast-forward rejection); left to implementation (concurrency group or retry) since agentic merges are effectively serialized.
