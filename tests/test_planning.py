...
def test_planning_phase_all_blocked_hydrates_canonical_titles_for_number_only_entries(
    tmp_path, git_svc
):
    recording = RecordingStatusDisplay()
    output = PlannerOutput(issues=[], blocked=[{"number": 5}])
    issues = [
        {
            "number": 5,
            "title": "Unblock planner parsing",
...
def test_planning_phase_uses_resolved_blocked_entries_when_planner_selections_are_stale(
    tmp_path, git_svc
):
    recording = RecordingStatusDisplay()
    output = PlannerOutput(
