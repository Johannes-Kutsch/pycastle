...
- `PromptRenderer.__init__(cfg)` reads `coding-standards/*.md`, composes globals from `cfg`, iterates `list(PromptTemplate)`, raises on any out-of-scope token. Failure aborts `pycastle run` with offending template + token.
