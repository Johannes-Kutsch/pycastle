...
- **The decision is a pure function**, taking facts and returning a named outcome, following the shape of the branch-setup resolution. It lives in the services layer beside the **remote retry decision** module — its structural sibling: a private, pure policy module owned by `GitService`. The branch-setup resolution itself sits in the iteration layer, which the services layer never imports, so placing it there would invert the dependency. It is deliberately not merged *into* the retry module: that one classifies *stderr from a failure*, and this decides *a state before anything fails*.
