...
| **PlainStatusDisplay** | Plain-terminal adapter for `StatusDisplay` defined in `status_display` module; panel methods (`add_agent`, `update_phase`, `remove_agent`, `reset_idle_timer`) are no-ops; `print` delegates to `builtins.print` so orchestrator progress messages reach the terminal when no rich live panel is active | NullStatusDisplay |
