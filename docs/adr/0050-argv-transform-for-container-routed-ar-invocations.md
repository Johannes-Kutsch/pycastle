...
Pycastle uses this to route provider invocations into a Docker container: the transform prepends `docker exec <container_id>` and injects required env vars via `-e KEY=VALUE` flags. `ContainerRunner` becomes a thin Docker lifecycle wrapper — start container → supply `argv_transform` closure over `container_id` → tear down.
