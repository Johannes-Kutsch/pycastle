...
- **429** → existing `UsageLimitError` path (account-specific; `mark_exhausted`; sleep to reset). Unchanged.
- **`api_error_status >= 500`, or `is_error: true` with no `api_error_status`** → new transient-error exception. Worktree preserved; sibling `CancellationToken` cancelled so not-yet-started parallel agents in the same iteration bail; agents already executing in `runner.work` finish; no `mark_exhausted`. Orchestrator continues to the next iteration where in-flight detection re-spawns the failed agent against the preserved worktree.
- **4xx other than 429** → new hard-error exception. Worktree preserved; sibling `CancellationToken` cancelled; running siblings finish; one structured bug is filed via the existing `auto_file_issue` helper (per ADR 0022). The iteration boundary records the hard error and the orchestrator exits non-zero after the current iteration rather than entering iteration N+1.

The detector lives at the parsing layer, so the rule applies uniformly across every `AgentRole` — Planner, Implementer, Reviewer, Merger, PreflightIssue, Improve, DivergenceResolver, FailureReport.

Trigger was issue #831: a Reviewer hit a 529 Overloaded after the CLI's internal retries were exhausted. The CLI emitted a synthetic terminal `result` envelope (`is_error: true, api_error_status: 529, stop_reason: stop_sequence`). The pre-existing `_check_usage_limit` only matched `api_error_status == 429`, so the line was parsed as a normal `Result` event. `_CommitMessageHandler.extract_final()` legitimately returned `CommitMessageOutput(message=None)` per ADR 0007, the orchestrator composed a synthetic `Review #443 - …` commit, flipped the stage-done sentinel, and the broken review was treated as complete on the next iteration via the review-skip path.
...
- **Three sibling exceptions at the parsing layer, branched policy at the iteration boundary — chosen.**
...
- `ClaudeService.run` yields three event variants for the failure cases — existing `UsageLimit` plus two new ones — and `process_stream_from_events` raises three sibling exception classes.
...
- The iteration boundary in `iteration/__init__.py` gains two new top-level catches per the ADR 0008 pattern: transient → next iteration with no sleep; hard → record + exit non-zero after current iteration.
- On the hard path, the iteration boundary calls `auto_file_issue` (ADR 0022) with title `[pycastle] Claude API <status>: <first line>`, labels `["bug", "needs-triage"]`, body carrying the raw `result` envelope and agent-role context. Gated by `auto_file_bugs`; falls back to prefilled `issues/new?…` URL when the gate is off.
...
- Stage-done sentinel never flips on either new path — the protocol raises before the `Result` event reaches a role handler, so role-session cleanup is skipped and `RoleSession.is_resumable()` remains true.
- Cross-role behaviour change: Planner / Merger / PreflightIssue / Improve / DivergenceResolver on a 5xx no longer raise `PromiseParseError` / `PlanParseError` / `IssueParseError` — they raise the transient exception, lifting them into the same preserve-and-retry semantics implementers and reviewers get. Same lift applies to the 4xx path: those roles now auto-file rather than crashing the orchestrator.
