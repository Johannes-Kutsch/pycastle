...
# ── Behavior 11: cron is a hidden deprecated alias for run ────────────────────


def test_cron_cmd_not_in_help(tmp_path, monkeypatch):
    from pycastle.main import main as cli
...
def test_cron_cmd_prints_deprecation_notice_and_delegates_to_run(tmp_path, monkeypatch):
    from pycastle.main import main as cli
    ...
    assert result.exit_code == 0, result.output
    assert "deprecated" in result.output.lower() or "pycastle run" in result.output


def test_cron_cmd_accepts_no_improve_option(tmp_path, monkeypatch):
    from pycastle.main import main as cli
