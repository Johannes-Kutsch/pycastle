...
def test_get_parent_reads_real_parent_issue_url_field():
    # The real GitHub REST issue payload exposes the parent as a
    # ``parent_issue_url`` string, not a ``parent`` object.
    transport = _ScriptedGithubTransport(
        [
            _script_step(
                "GET",
                ...
                payload={
                    "number": 5,
                    "parent_issue_url": (
                        "https://api.github.com/repos/owner/repo/issues/100"
                    ),
                },
            )
        ]
...
def test_close_issue_with_parents_warns_and_stops_recursive_parent_chain_when_parent_lookup_fails():
    transport = _ScriptedGithubTransport(
        [
            _script_step(
                "PATCH",
    ...
    with pytest.warns(
        UserWarning,
        match=r"Unable to continue parent cascade after closing issue #50: boom",
    ):
        svc.close_issue_with_parents(5)
