...
| **StatusDisplay** | Injectable abstraction that owns the live terminal status line; exposes `add_agent`, `update_phase`, `update_message`, `reset_idle_timer`, `remove_agent`, and `print` methods; backed by a `rich` `Live` display in production and a `NullStatusDisplay` no-op in tests; injected via `Deps` as a separate concern from `Logger` | terminal display, status bar |
| **NullStatusDisplay** | Test/no-op implementation of `StatusDisplay`; all methods are no-ops; `print` falls back to `builtins.print` | — |
| **agent status row** | One headerless line in the `StatusDisplay` live panel representing one active agent; format: `elapsed \| AgentName \| Phase \| idle Last message…`; elapsed is dim and right-justified so all agent names align across rows; agent name is bold and an OSC 8 clickable link to the log file, with any numeric part (e.g. `5` in `Implementer #5`) styled cyan to match the Rich console highlighter; phase is colored by stage (plan=blue, implement=orange, review=yellow, merge=green); idle is dim; last message renders Rich markup and shows only the first line, truncated with ellipsis to fit terminal width; elapsed counts up from agent start and never resets; idle resets on each Docker stream chunk; the live panel is preceded by one blank line to visually separate it from scrollback output; ordered by orchestration phase (plan → implement → review → merge) then by issue number; disappears when the agent finishes | status entry, agent row |
| **OSC 8 link** | Terminal hyperlink using the OSC 8 ANSI escape sequence (`\033]8;;file:///path\033\\text\033]8;;\033\\`); used in the agent status row on the agent name; link text is the agent name | terminal link, clickable link |
...
- The streaming loop calls `reset_idle_timer` on every Docker chunk (matching the kill-agent timeout behavior); per completed line, `_format_stream_line` is called — if it returns a non-None string, `update_message` is called with the first line of that string; tool-use blocks in assistant messages are suppressed (return `None`); `update_message` also resets the idle timer; the status row is the sole terminal surface for active agents.
