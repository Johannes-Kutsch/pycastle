...
2. **Runtime adaptation** — mechanics the orchestrator owns: label placeholders instead of literal label names, AFK-only framing in improve mode, one-pass layer-count resolution (a phase agent cannot recursively re-invoke `/to-tickets`), the CONTEXT.md-issue-is-a-docs-slice rule, no sub-agent mechanics (prompts must stay provider-agnostic), and condensation of concerns the orchestrator enforces mechanically (e.g. the frozen-clock fixture prose — guaranteed by `tests/conftest.py` and the `frozen-clock` preflight check, so the port keeps only the rules an agent can still break).
